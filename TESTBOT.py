...
# идеи: 1. Сделать меню для бота чтобы юзеры могли своих ботов делать
# 2. настроить нормально поддержку стикеров (добавлять их либо на сервер пустышку, либо отправлять в правильных форматах)
# 3. сделать норм перенос файлов из тг в дс
...
async def telegram_message(message: types.Message):
    ...
    async with aiohttp.ClientSession() as session:
        webhook: Webhook = Webhook.from_url("", session=session)
        ...
        if message.text:
            ...
            if message.sticker.is_animated:
                await send_file(message.sticker.file_id, f"sticker_{message.sticker.file_id}.webm")
            else:
